...
int YMesh::get_index_list() {
	if(ib_ == NULL) {
		...
		if(primitive_type_ == D3DPT_TRIANGLELIST)
			init_vertex_count_ = 3 * prim_count_;
		else if(primitive_type_ == D3DPT_TRIANGLESTRIP)
			init_vertex_count_ = prim_count_ + 2;
		else
			init_vertex_count_ = 0;
...
void YMesh::dump_index() {
	...
	if(primitive_type_ == D3DPT_TRIANGLELIST) {
		...
		int cnt = min(indices_.size() - 2, prim_count_);
...
void YMesh::select_vertex() {
	...
	if(!need_decimate()) {
		...
		Log::log("vertex: ");
		for(int i=0; i<init_vertex_count_; ++i) {
			remain_vertices_.push_back(i + base_vertex_index_);
			Log::log("%d, ", i + base_vertex_index_);
		}
		Log::log("\n");
	}
	else {
...
void YMesh::update_vertex_buffer(int sn) {
	...
	//如果当前帧为奇数帧，则更新VertexBuffer，否则不更新
	if( cur_frame_ == 0 ) {
		goto RET;
	}
	...
	*/
	...
	cs.write_int(vb_->offsetInBytes);
	...
	for(int i=0; i<remain_vertices_.size(); ++i) {
		...
		if(bs_.is_set(i)) {
			...
			vt = sv_vt + remain_vertices_[i] * vb_->stride + vb_->offsetInBytes;
...
void YMesh::select_pos_change() {
	...
	pos += (remain_vertices_[0] * pos_vb_->stride + pos_vb_->offsetInBytes);
...
void YMesh::create_index_buffer(int idx_count, YSlim* slim, WrapperDirect3DIndexBuffer9** ib) {
	...
	device_->CreateIndexBuffer(idx_size * idx_count, usage, D3DFMT_INDEX32, pool, (IDirect3DIndexBuffer9**)&tmp_ib, NULL);
