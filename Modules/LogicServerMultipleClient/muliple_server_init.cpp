...
#include "muliple_wrap_direct3d9.h"

#include "muliple_wrap_direct3d9.h"
#include "muliple_wrap_direct3dvertexbuffer9.h"
#include "muliple_wrap_direct3dindexbuffer9.h"
#include "muliple_wrap_direct3dvertexdeclaration9.h"
#include "muliple_wrap_direct3dvertexshader9.h"
#include "muliple_wrap_direct3dpixelshader9.h"
#include "muliple_wrap_direct3dtexture9.h"
#include "muliple_wrap_direct3dstateblock9.h"
#include "muliple_wrap_direct3ddevice9.h"
#include "muliple_wrap_direct3dcubetexture9.h"
#include "muliple_wrap_direct3dswapchain9.h"
#include "muliple_wrap_direct3dsurface9.h"

#include "muliple_ymesh.h"
...
HWND WINAPI CreateWindowCallback(DWORD dwExStyle,LPCSTR lpClassName,LPCSTR lpWindowName, DWORD dwStyle,int x,int y,int nWidth,int nHeight,HWND hWndParent, HMENU hMenu,HINSTANCE hInstance,LPVOID lpParam) {
	Log::log("CreateWindowCallback() called, width:%d, height:%d\n", nWidth, nHeight);
...
HWND WINAPI CreateWindowExWCallback( DWORD dwExStyle, LPCWSTR lpClassName, LPCWSTR lpWindowName, DWORD dwStyle,
 int X, int Y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam) {
	Log::log("CreateWindowExWCallback() called, Widht:%d, Height:%d\n", nWidth, nHeight);
...
void GetSocketsFromSharedFileMap(){
	Log::log("GetSocketsFromSharedFileMap.\n");
	char * cmdLine;
	cmdLine = GetCommandLine();
	int len = strlen(cmdLine);

	if (len >= 100){
		Log::log("cmdline is too long.\n");
		return;
	}
	else{
...
void GetSocketsFromCmd() {
	...
	if(len < 100) {
		...
		if(command_socket_handle == -1) {
			...
			Log::slog("[server init]: error command socket handle.\n");
			...
			Log::log("[server init]: new command socket handle:%d.\n", cs.get_connect_socket());
		}
...
void WINAPI ExitProcessCallback(UINT uExitCode) {
	...
	Log::slog("WrapperDirect3D9::ins_count = %d\n", WrapperDirect3D9::ins_count);
	Log::slog("WrapperDirect3DDevice9::ins_count = %d\n", WrapperDirect3DDevice9::ins_count);
	Log::slog("WrapperDirect3DVertexBuffer9::ins_count = %d\n", WrapperDirect3DVertexBuffer9::ins_count);
	Log::slog("WrapperDirect3DIndexBuffer9::ins_count = %d\n", WrapperDirect3DIndexBuffer9::ins_count);
	Log::slog("WrapperDirect3DVertexDeclaration9::ins_count = %d\n", WrapperDirect3DVertexDeclaration9::ins_count);
	Log::slog("WrapperDirect3DVertexShader9::ins_count = %d\n", WrapperDirect3DVertexShader9::ins_count);
	Log::slog("WrapperDirect3DPixelShader9::ins_count = %d\n", WrapperDirect3DPixelShader9::ins_count);
	Log::slog("WrapperDirect3DTexture9::ins_count = %d\n", WrapperDirect3DTexture9::ins_count);
	Log::slog("WrapperDirect3DStateBlock9::ins_count = %d\n", WrapperDirect3DStateBlock9::ins_count);
	Log::slog("WrapperDirect3DCubeTexture9::ins_count = %d\n", WrapperDirect3DCubeTexture9::ins_count);
	Log::slog("WrapperDirect3DSwapChain9::ins_count = %d\n", WrapperDirect3DSwapChain9::ins_count);
	Log::slog("WrapperDirect3DSurface9::ins_count = %d\n", WrapperDirect3DSurface9::ins_count);
