...
#include "muliple_wrap_direct3dvolumetexture9.h"
#include "opcode.h"
...
STDMETHODIMP WrapperDirect3DDevice9::DrawPrimitive(THIS_ D3DPRIMITIVETYPE PrimitiveType,UINT StartVertex,UINT PrimitiveCount) {
	Log::log("WrapperDirect3DDevice9::DrawPrimitive(), vb_0=0x%X, PrimitiveType=%d, startVertex=%d, primitiveCount=%d\n", cur_vbs_[0], PrimitiveType, StartVertex, PrimitiveCount);
	...
	if(true) {
		...
		y_mesh = cur_vbs_[0]->get_y_mesh(StartVertex, PrimitiveCount);
		...
		if(y_mesh == NULL) {
			...
			cur_vbs_[0]->put_y_mesh(StartVertex, PrimitiveCount, y_mesh);
			...
			y_mesh->set_index_params(PrimitiveType, StartVertex, 0, 0, 0, PrimitiveCount);
		}
		else {
	...
	if(y_mesh->decision_ == NO_NEED) {
		//Log::slog("WrapperDirect3DDevice9::DrawPrimitive(), NONEED\n");
...
STDMETHODIMP WrapperDirect3DDevice9::DrawPrimitiveUP(THIS_ D3DPRIMITIVETYPE PrimitiveType,UINT PrimitiveCount,CONST void* pVertexStreamZeroData,UINT VertexStreamZeroStride) {
	...
	*/
	...
	//Log::slog("DrawPrimitiveUp: vertex count:%d, zero stride:%d.\n", VertexCount, VertexStreamZeroStride);
