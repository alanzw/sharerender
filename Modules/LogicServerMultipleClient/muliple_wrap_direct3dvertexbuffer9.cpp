...
YMesh* WrapperDirect3DVertexBuffer9::get_y_mesh(UINT StartVertex, UINT PrimitiveCount) {
	memset(v_mesh_arr, 0, sizeof v_mesh_arr);
	v_mesh_arr[0] = id;
	v_mesh_arr[1] = StartVertex;
	v_mesh_arr[2] = PrimitiveCount;
	v_mesh_arr[3] = this->offsetInBytes;

	YashNode* nd = mesh_table_.look_up( Slice((char*)v_mesh_arr, 16) );

	if(nd == NULL) return NULL;
	else return (YMesh*)(nd->val);
}

void WrapperDirect3DVertexBuffer9::put_y_mesh(UINT StartVertex, UINT PrimitiveCount, YMesh* y_mesh) {
	memset(v_mesh_arr, 0, sizeof v_mesh_arr);
	v_mesh_arr[0] = id;
	v_mesh_arr[1] = StartVertex;
	v_mesh_arr[2] = PrimitiveCount;
	...
	mesh_table_.insert( Slice((char*)v_mesh_arr, 16), y_mesh );
}
