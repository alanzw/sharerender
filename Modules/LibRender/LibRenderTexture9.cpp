...
ClientTexture9::ClientTexture9(IDirect3DTexture9* ptr): m_tex(ptr) {
	cg::core::infoRecorder->logTrace("serverTexture9 constructor called!\n");
}

HRESULT ClientTexture9::FillData(int Level, int Pitch, int size, void* ptr) {
	cg::core::infoRecorder->logTrace("ClientTexture9::FillData() called, this:%d\n",this);
	...
	cg::core::infoRecorder->logTrace("ClientTexture9::FillData() surface return:%d\n",top_surface);
	if(top_surface == NULL)
		return D3D_OK;
