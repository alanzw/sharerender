...
class Encoder: public CThread{
	...
	int encoder_height, encoder_width;
...
public: 
	...
	inline void setSrcPipeName(char * name){ pipName = _strdup(name); }
	...
	// no use
	int init(RTSPConf * conf, pipeline * pipe){}
	RTSPConf * conf;
	inline void setRTSPConf(RTSPConf * _conf){ conf = _conf; }
	RTSPContext * context;
	void setRTSPContext(RTSPContext * _context){ context = _context; }
};
