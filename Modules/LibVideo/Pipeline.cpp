...
pipeline::pipeline(int privdata_size): PipelineBase(privdata_size) {
	...
	infoRecorder->logTrace("[pipeline]: constructor called, name:%s.\n", t);
	do_register(t, this);
#if 0
...
pipeline::pipeline(int privdata_size){
	...
	if(poolmutex == NULL){
		infoRecorder->logTrace("[pipeline]: pool mutex creation failed.\n");
	}
	WaitForSingleObject(poolmutex, INFINITE);
	ReleaseMutex(poolmutex);

	infoRecorder->logTrace("[pipeline]: pool mutex:%p.\n", poolmutex);
	//DebugBreak();
	int i = 0; 
	i++;
	bufpool = NULL;
...
pipeline::store_data(struct pooldata *data) {
	...
	datacount++;
	infoRecorder->logError("[pipeline]: stored data, data count:%d\n", datacount);
	ReleaseMutex(poolmutex);
	return;
}
...
struct pooldata *
	pipeline::load_data_unlocked() {
		...
		infoRecorder->logError("[pipeline]: to load data, data count:%d\n", datacount);
	
		if(datacount == 0){
			infoRecorder->logError("[pipeline]: error.\n");
		}
		struct pooldata *data;
		if (datatail == NULL) {
			return NULL;
...
pipeline::wait(HANDLE cond, HANDLE mutex) {
	...
	if (ret == WAIT_OBJECT_0){
		infoRecorder->logTrace("[pipeline]: wait single is signed.\n");
		infoRecorder->logTrace("[pipeline]: pipe %s wait single is signed.\n", name());
	}
	else if (WAIT_FAILED == ret){
		infoRecorder->logTrace("[pipeline]: wait failed. last error:%d.\n", GetLastError());
...
#else
	infoRecorder->logTrace("[pipeline]: notifier event:0x%p.\n", waitEvent);
