...
int D3DWrapper::init(struct ccgImage *image, HWND captureHwnd){
	...
	if (!g_pD3D){
		...
		{
			infoRecorder->logError("Unable to Create Direct3D\n");
			return -1;
		}
	}
	...
	{
		infoRecorder->logError("Unable to Get Adapter Display Mode\n");
		goto initErrorQuit;
	}
	...
	{
		infoRecorder->logError("Unable to Create Device\n");
		goto initErrorQuit;
	}
	...
	{
		infoRecorder->logError("Unable to Create Surface\n");
		goto initErrorQuit;
	}
...
bool D3DWrapper::capture(LARGE_INTEGER captureTv, LARGE_INTEGER initialTv, LARGE_INTEGER freq, int frameInterval){
	...
	if(this->isWrapSurface()){
		//DebugBreak();
		infoRecorder->logTrace("[D3DWrapper]: cpature surface.\n");
		// get the surface, store to cuda pipeline
		// cheeck the device
		if(this->d3d_device == NULL){
			infoRecorder->logError("[D3DWrapper]: get NULL d3d device.\n");
			return false;
		}
		if(this->cudaPipe == NULL){
			infoRecorder->logError("[D3DWrapper]: get NULL piepline.\n");
			return false;
		}
		if(frameInterval == 0){
			frameInterval = 2;
		...
		if(this->capture(CUDA_ENCODER, sframe)){
			...
			cudaPipe->notify_all();
			infoRecorder->logTrace("[D3DWrapper]: store surface %p, type addr:%p, vframe type addr:%p, DXSurface:0x%p.\n", data->ptr, &(sframe->type), &(vframe->type), sframe->dxSurface->d10Surface);


			///infoRecorder->logTrace("[D3DWrapper]: 
		}else{
			// failed.
			infoRecorder->logError("[D3DWrapper]: capture NULL surface for D3DWrapper.\n");
			return false;
		}
	}
	else{
		...
		if(!d3d_device || !pipe){
			infoRecorder->logError("[D3DWrapper]: get NULL pipeline.\n");
		}
		...
		if(result == NULL){
			infoRecorder->logError("[D3DWrapper]: capture NULL surface for image.\n");
			return false;
		}
		result->GetDesc(&desc);
		HRESULT hr = result->LockRect(&lockedRect, NULL, NULL);
		if(FAILED(hr)){
			infoRecorder->logError("[D3DWrapper]: lock rect failed.\n");
			return false;
		}
		// copy image
		do{
			...
			infoRecorder->logTrace("[D3DWrapper]: get image data:%d, height:%d, pitch:%d.\n", imageSize, iframe->realHeight, lockedRect.Pitch);
...
int D3DWrapper::capture(char * buf, int bufLen, struct ccgRect * gRect){
	...
	{
		infoRecorder->logError("Unable to Lock Front Buffer Surface\n");
		return -1;
	}
	// copy
	if (gRect == NULL) {
...
IDirect3DSurface9 * D3DWrapper::capture(ENCODER_TYPE encoderType, IDirect3DSurface9 ** ppDst){
	...
	infoRecorder->logTrace("none type addr:%p, VFrame type:%p.\n", &(no->type), &(vf->type));
#endif
	...
	if (desc.MultiSampleType != D3DMULTISAMPLE_NONE){
		if (resolvedSurface == NULL){
			...
			if (FAILED(hr)){
				...
				infoRecorder->logTrace("[D3DWrapper]: CreateRenderTarget (resolvedSurface) failed.\n");
		...
		infoRecorder->logTrace("[D3DWrapper]: get the render target. deviceOffScreenSurface:0x%p.\n", deviceOffscreenSurface);
	}
	else{
		infoRecorder->logTrace("[capture]: render target is not Multisampled.\n");
		infoRecorder->logTrace("[D3DWrapper]: render target is not Multisampled.\n");
	...
	if(encoderType == ENCODER_TYPE::X264_ENCODER){
		...
		if(deviceOffscreenSurface == NULL){
			infoRecorder->logTrace("[D3DWrapper]: device offscreen surface is NULL? error ?\n");
		...
		if(FAILED(hrr)){
			infoRecorder->logTrace("[D3DWrapper]: save surface to file failed.\n");
		}
#endif
		//
#if 1
...
bool D3DWrapper::capture(ENCODER_TYPE encoderType, SurfaceFrame * sframe){
	...
	// create a new RenderTarget no matter whether the render target is MULTI_SAMPLED or not
	if (resolvedSurface == NULL){
		infoRecorder->logTrace("[capture]: to create device surface for pipeline, total should be 8!\n");
		hr = d3d_device->CreateRenderTarget(desc.Width, desc.Height, desc.Format, D3DMULTISAMPLE_NONE,
			0, FALSE, &resolvedSurface, NULL);
		if (FAILED(hr)){
			infoRecorder->logTrace("[capture]: CreateRenderTarget (resolvedSurface) failed.\n");
...
int WindowWrapper::init(struct ccgImage *image, HWND _captureHwnd){
	...
	if (captureCompatibaleDC == NULL || captureCompatibleBitmap == NULL){
		infoRecorder->logError("[WindowWrapper]: unbale to create compatibale DC/Bitmap.\n");
		return -1;
	}
...
bool WindowWrapper::capture(LARGE_INTEGER captureTv, LARGE_INTEGER initialTv, LARGE_INTEGER freq, int frameInterval){
	infoRecorder->logTrace("[WindowWrapper]: this capture should never be used.\n");
	return false;
}
...
DWORD WINAPI WindowWrapper::WrapperThreadProc(LPVOID param){
	...
	if (!wrapper || !wrapper->captureHwnd){
		infoRecorder->logError("[WindowWrapper]: get NULL wrapper or NULL window handle in wrapper thread proc.\n");
		return -1;
	}
...
int D3D11Wrapper::capture(char * buf, int bufLen, struct ccgRect * gRect){
	// do nothing
	infoRecorder->logTrace("[D3D11Wrapper]: should never use this capture function.\n");
	return -1;
}
...
bool D3D11Wrapper::capture(LARGE_INTEGER captureTv, LARGE_INTEGER initialTv, LARGE_INTEGER freq, int frameInterval){
	...
	if(FAILED(hr)){
		...
		infoRecorder->logTrace("[D3D11Wrapper]: Failed to create buffer.\n");
		return ret;
	}
