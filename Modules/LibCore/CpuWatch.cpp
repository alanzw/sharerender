...
int64_t CpuWatch::last_sys_idle=0;
int64_t CpuWatch::last_sys_kernel=0;
int64_t CpuWatch::last_sys_user=0;
unsigned int CpuWatch::processor_num=0;
int64_t CpuWatch::last_time = 0;
int64_t CpuWatch::last_sys_time = 0;

#else
unsigned int CpuWatch::processor_num=0;
#endif
CpuWatch::CpuWatch(void): last_sys_time(0), last_sys_kernel(0), last_sys_idle(0), last_time(0), last_sys_user(0)
{
	memset(processName,0,sizeof(processName));
}

CpuWatch::CpuWatch(char *procName): last_sys_time(0), last_sys_kernel(0), last_sys_idle(0), last_time(0), last_sys_user(0)
{
	//初始化进程名
...
double CpuWatch::GetProcessCpuUtilization(HANDLE proc){
