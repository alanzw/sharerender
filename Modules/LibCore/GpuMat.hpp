...
namespace cg{
	namespace core{
		...
		template<typename T> struct Ptr{
			...
			// returns a ptr that owns the same pointer as this, and stores the stame pointers as this, except converted via static_cast to Y *.
			template<typename Y>
			Ptr<Y> staticCast() const;
			// ditto for const_cast
			template<typename Y>
			Ptr<Y> constCast() const;
			// ditto for dynameic_cast
			template<typename Y>
		...
		// here we define the CUDA Runtime API, if not define, use Driver API as default
