	...
	namespace core{
		...
		public:
			...
			~Cache() {
				...
				if(helperHandle){
					TerminateThread(helperHandle, 0);
					helperHandle = NULL;
				}
				if(pSection){
			...
			static inline UINT hash_slice(const Slice& s) {
				return cg::core::hash(s.data(), s.size(), 0);
			}
			...
			void lru_release(HashNode* e) {
				...
				if((end - start )  > 0){
					infoRecorder->logError("free cast:%d ms.\n", end -start);
				}
#else
				infoRecorder->logError("[Cache]: to release %p.\n", e);
				FreeMem(e);
#endif
			}
			...
			float getHitRate(){
				float ret = 0.0f;
				ret = hit_cnt * 1.0f / (hit_cnt + set_cnt);
				return ret;
			}
			...
			static void WINAPI startHelper();
			static DWORD WINAPI CacheHelper(LPVOID param);
			static void WINAPI FreeMem(void * p);

			static list<void *> gcMem;
			static CRITICAL_SECTION section;
			static LPCRITICAL_SECTION pSection;

			static HANDLE helperHandle;
			static DWORD helperId;
