...
// all gc concern is in namespace GC
namespace cg{
	namespace core{
		...
		// Exception thrown when an attempt is make to
		...
		// An iterator-like class ofr cycling through arrays that are pointed
		// to by GCPtrs. Iter pointers ** do not ** participate in or affect 
		// garbage collection. Thus, an Iter pointing to some object does not 
		// prevent that object from being recycled.

		template <class T> class Iter{
			T * ptr;	// current point value
			T * end;	// points to element one past end
			T * begin;	// points of start of allocated array
		...
		public:
			...
			// Do not allow out-out-bounds access
			T * operator->(){
				if((ptr >= end) || (ptr < begin)){
					throw OutOfRangeExc();
				}
			...
			// Return a reference to the objeect at the specificed index. Do not
		...
		public:
			...
			T * memPtr;			// pointer to allocate memory

			/* isArray is true if memPtr points to an allocated array. it is false otherwise */
			bool isArray;			// true if pointing to array

			/* if memPtr is pointing to an allocated array, then arraySzie contains its size */
			unsigned arraySize;	// the size of the array

			// Here, mPtr points to the ll9cated memroy
			// If this is an array, then size specifies the size of array.
		...
		// Overlaoding operator== allows GCInfos to be compared.
		// This is need by the STL list class.
		template <class T> bool operator==(const GCInfo<T> &ob1, const GCInfo<T> &ob2){
			return (ob1.memPtr == ob2.memPtr);
		}

		// GCPtr implements a pointer type that uses garbage collection to release
		// memory. A GCPtr must only be used to point to memory that was dynamically 
		// allocated using new. When used to refer to an allocated array, specify 
		// array the array size.
		template<class T, int size = 0> class GCPtr{
			// gclist maintains the garbage collection list.
			static list<GCInfo<T> > gclist;
			// addr points to the allocated memory to which this GCPtr pointer 
			// currently points
			T * addr;
			...
			// if this GCPtr is pointing to an allocated array, then arraySize contains 
			// its size.
		...
		public:
			...
			GCPtr(T * t = NULL){
				...
				// If t is already in gclist, then increment its refcount
				// Otherwise, add it to the list
				if(p != gclist.end())
					p->refcount++; // increment the ref count
				else{
			...
			// Collect garbage, Returns true is at least one object was freed.
			static bool collect();
			...
			// Return a Iter to the start of the allocated memory.
			Iter<T> begin(){
				int size;
			...
			// Clear gclist when program exist.
		...
		GCPtr<T, size>::~GCPtr(){
			...
			// such as after gclist has reached a certain size, after a certain number of GCPtrs have gone out of scope, or when memory is low.
		}


		// Collect the garbage. Return true if at least one object was freed.
		template <class T, int size>
		...
		T * GCPtr<T, size>::operator=(T * t){
			list<GCInfo<T> >::iterator p;
			// First decrement the reference count for the memory currently being 
			// Pointed to.
			p = findPtrInfo(addr);
			p->refcount--;

			// Next, if the new address is ready existence in the system, increment
			// it count. Otherwise, create a new entry for gclist.
			p = findPtrInfo(t);
			if(p != gclist.end())
				p->refcount++;
			else{
				// Crate and store this entry
				GCInfo<T> gcObj(t, size);
				gclist.push_front(gcObj);
			}
		...
		void GCPtr<T, size>::shutdown(){
			if(gclistSize() == 0) return;		// list empty
			list<GCInfo<T> >::iterator p;
...
#ifdef DISPLAY
			cout << "Before collecting for shutdown() for " << typeid(T).name() << endl;
#endif
			collect();
#ifdef DISPLAY
