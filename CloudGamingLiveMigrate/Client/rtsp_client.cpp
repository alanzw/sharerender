...
#include "../LibVideo/VSource.h"
#endif
#include "../LibVideo/RtspClient.h"


#include "../LibVideo/AVCodecCommon.h"
	...
	for (;;) {
		if (q->queue.size() > 0) {
			*pkt = q->queue.front();
			...
			DWORD r = WaitForSingleObject(q->cond, ts.tv_sec * 1000);
			if (r == WAIT_TIMEOUT){
...
init_vdecoder(int channel, const char *sprop) {
	...
	if ((names = ccgClient::LookupDecoders(video_codec_name)) == NULL) {
		rtsperror("video decoder: cannot find decoder names for %s\n", video_codec_name);
		return -1;
	}
	video_codec_id = ccgClient::LookupCodecID(video_codec_name);
...
init_adecoder() {
	...
	if ((names = ccgClient::LookupDecoders(audio_codec_name)) == NULL) {
		rtsperror("audio decoder: cannot find decoder names for %s\n", audio_codec_name);
		return -1;
	}
	audio_codec_id = ccgClient::LookupCodecID(audio_codec_name);
	if ((codec = AVCodecCommon::AVCodecFindDecoder(names, AV_CODEC_ID_NONE)) == NULL) {
...
play_video_priv(int ch/*channel*/, unsigned char *buffer, int bufsize, struct timeval pts) {
	...
	avpkt.data = buffer;
	while (avpkt.size > 0) {
		//the vframe is the dstination frame(decoded), the avpkt is the source frame
		...
		if (got_picture) {
			...
			//WaitForSingleObject(rtspParam->surfaceMutex[ch], INFINITE);
			EnterCriticalSection(&(rtspParam->surfaceMutex[ch]));
			if (rtspParam->swsctx[ch] == NULL) {
				...
				//ReleaseMutex(rtspParam->surfaceMutex[ch]);
				LeaveCriticalSection(&(rtspParam->surfaceMutex[ch]));
...
DWORD WINAPI rtsp_thread(LPVOID param) {
	...
	rtspClientCount = 0;
	//
	rtspconf = RTSPConf::GetRTSPConf();// rtspconf_global();
	rtspParam = (RTSPThreadParam*)param;
	infoRecorder->logError("[client]: rtsp thread param: url = %s. running = %d.\n", rtspParam->url, rtspParam->running);
	...
	do if (scs.subsession != NULL) {
		if (!scs.subsession->initiate()) {
			...
			if (strcmp("video", scs.subsession->mediumName()) == 0) {
				...
				video_codec_name = _strdup(scs.subsession->codecName());
				...
				audio_codec_name = _strdup(scs.subsession->codecName());
#ifdef ANDROID
...
struct timeval presentationTime, unsigned /*durationInMicroseconds*/) {
	...
	if (fSubsession.rtpPayloadFormat() == video_sess_fmt) {
		...
		//TODO : here ? check the frame index ?
