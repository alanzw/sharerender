...
int VideoGenerator::loopFor2DGame(){
	...
	if(rtspConf == NULL){
		infoRecorder->logError("[VideoGenerator]: get NULL rtspConfig.\n");
		return -1;
	}
	...
	if(getCurSourceType() == SURFACE){
		infoRecorder->logError("[VideoGenerator]: 2D game do not support SURFACE.\n");
		return -1;
	}
	...
	if(this->filter){
		...
		// null filter, error
		infoRecorder->logError("[VideoGenerator]: NULL filter.\n");
		return -1;
	}
	...
	if(this->x264Encoder){
		...
		// null encoder, error
		infoRecorder->logError("[VideoGenerator]: NULL x264Encoder.\n");
		return -1;
	}
...
int VideoGenerator::loopForD3DGame(){
	...
	while(isRunning()){
		...
		if(presentEvent == NULL){
			infoRecorder->logError("[VideoGenerator]: NULL present event for 3D games.\n");
		}
#endif
		if(presentEvent)
		...
		if(dxVersion == DX9){
			...
			dxWrapper->d9Wrapper->capture();
		}
		else if(dxVersion == DX10){
			dxWrapper->d10Wrapper->capture();
		}else if(dxVersion == DX10_1){
			dxWrapper->d10Wrapper1->capture();
		}else if(dxVersion == DX11){
			dxWrapper->d11Wrapper->capture();
		}
...
bool VideoGenerator::doCapture(IDirect3DDevice9 * device, RTSPConf * rtspConf, LARGE_INTEGER captureTv, LARGE_INTEGER initialTv, LARGE_INTEGER freq, int frame_interval){
	...
	dxWrapper->d9Wrapper->captureInit(rtspConf->video_fps);

	capturedSurface = dxWrapper->d9Wrapper->capture(this->getCurEncoderType(), this->windowRect->width, this->windowRect->height);
	...
	//else if(this->sourceType == SOURCE_TYPE::SURFACE){
		...
		data = surfaceSource[0]->allocate_data();
		...
		if(sur == NULL){
			...
			sframe->setD3D9Surface(sur, windowRect->width, windowRect->height);
		}
		...
		for(i = 0; i< SOURCES; i++){
			struct pooldata * dupdata = surfaceSource[i]->allocate_data();
			...
			surfaceSource[i]->store_data(dupdata);
			surfaceSource[i]->notify_all();
		}
		surfaceSource[0]->store_data(data);
		infoRecorder->logTrace("[Channel]: source cudapipeline notify the client.\n");
		surfaceSource[0]->notify_all();
...
DWORD VideoGenerator::VideoGenertorThreadProc(LPVOID param){
	...
	infoRecorder->logError("[VideoGenerator]: enter the thread proc.\n");
	...
	infoRecorder->logError("[VideoGenerator]: exit the video streaming.\n");
	return 0;
}
#endif
...
int VideoGenerator::initVideoGenerator(){
	...
	if(SetWindowPos(captureWindow, HWND_TOP, 0, 0, 0, SWP_NOSIZE | SWP_SHOWWINDOW) == 0){
		infoRecorder->logError("[VideoGenerator]: Set window pos failed.\n");
		return -1;
	}
#else
...
int VideoGenerator::setAndSetupWindow(HWND hwnd){
	...
	if(SetWindowPos(hwnd, HWND_TOP, 0, 0, 0, 0, SWP_NOSIZE|SWP_SHOWWINDOW) == 0){
		infoRecorder->logError("[VideoGenerator]: SetWindowPos failed.\n");
		return -1;
	}
#endif

	if(GetClientRect(hwnd, &rect) == 0){
		infoRecorder->logError("[VideoGenerator]: GetClientRect failed.\n");
		return -1;
	}
	lt.x = rect.left;
	lt.y = rect.top;
	...
	if(ClientToScreen(hwnd, &lt) == 0 || ClientToScreen(hwnd, &rb) == 0){
		infoRecorder->logError("[VideoGenerator]: map from client coordinate to screen coordingate failed.\n");
		return -1;
	}

	//windowRect = new ccgRect();
	if(windowRect == NULL){
		infoRecorder->logError("[VideoGenerator]: window rect is NULL.\n");
		//return -1;
		windowRect = new ccgRect();
	}
	...
	if(windowRect->left < 0 || windowRect->top < 0){
		infoRecorder->logError("[VideoGenerator]: Invalid window: (%d, %d) - (%d, %d).\n", windowRect->left, windowRect->top, windowRect->right, windowRect->bottom);
		return -1;
	}
...
int VideoGenerator::setupCudaPipeline(){
	...
	for(int i = 0; i< SOURCES; i++){
		if((surfaceSource[i] = new pipeline()) == NULL){
			infoRecorder->logError("[VideoGenerator]: init pipeline for cuda failed.\n");
			return -1;
		}


		if((data = surfaceSource[i]->datapool_init(POOLSIZE, sizeof(struct SurfaceFrame))) == NULL){
				infoRecorder->logError("[VideoGenerator]: cannot allocate data pool.\n");
			...
			for(; data != NULL; data = data->next){
				...
				if(f->init() == false){
					infoRecorder->logError("[VideoGenerator]: init frame failed.\n");
					return -1;
				}
			}
	...
	do{
		...
		for(int i = 0; i < SOURCES; i++){
			...
			if((imageSource[i] = new pipeline()) == NULL){
				infoRecorder->logError("[VideoGenerator]: init pipeline failed.\n");
				return -1;
			}
...
int VideoGenerator::initVideoGenerator(HWND hwnd, int height, int width){
	...
	if(windowWrapper->init(image, hwnd) < 0){
		infoRecorder->logError("[VideoGenerator]: window wrapper init failed.\n");
		return -1;
	}
...
int VideoGenerator::setupFilterPipeline(){
	...
	infoRecorder->logTrace("[VideoGenerator]: init the filter pipeline.\n");
	...
	if(!pipe){
		if((pipe = new pipeline()) == NULL){
			infoRecorder->logError("[VideoGenerator]: cannot init filter pipeline.\n");
			return -1;
		}
	}
	imageDst[0]= pipe;
	...
	vsourceConf->maxStride = outputWidth << 2;
	if(pipe->alloc_privdata(sizeof(struct VsourceConfig)) == NULL){
		infoRecorder->logError("[VideoGenerator]: cannot allocate the private data for dst pipeline.\n");
		return -1;
	}
	...
	if((data = pipe->datapool_init(POOLSIZE, sizeof(struct ImageFrame))) == NULL){
		infoRecorder->logError("[VideoGenerator]: cannot allocate data pool for filter pipeline.\n");
		return -1;
	}
	// per frame init
	for(; data != NULL; data = data->next){
		...
		if(frame->init(outputHeight, outputWidth, vsourceConf->maxStride) == NULL){
			infoRecorder->logError("[VideoGenrator]: init frame for filter pipeline failed.\n");
...
int VideoGenerator::initVideoGenerator(HWND hwnd, DX_VERSION dxVer, void * device, int h, int w, int x, int y, int outH, int outW){
	...
	case DX9:
		...
		dxWrapper->d9Wrapper = new D3DWrapper((IDirect3DDevice9 *) device);
		dxWrapper->d9Wrapper->setHWND(hwnd);
	...
	case DX10:
		// init wrapper for image
		dxWrapper->d10Wrapper = new D3D10Wrapper((IDXGISwapChain *)device);
		dxWrapper->d10Wrapper->setHWND(hwnd);
		break;
	case DX10_1:
		// init the wrapper for image
		dxWrapper->d10Wrapper1 = new D3D10Wrapper1((IDXGISwapChain *)device);
		dxWrapper->d10Wrapper1->setHWND(hwnd);
		break;
	case DX11:

		// init the wrapper for image
		dxWrapper->d11Wrapper = new D3D11Wrapper((IDXGISwapChain * )device);
		dxWrapper->d11Wrapper->setHWND(hwnd);
		break;
	default:
		// use dx9
		this->dxVersion = DX9;
...
int VideoGenerator::initVideoGenerator(HWND hwnd, IDirect3DDevice9 *device, int height, int width, int x, int y){
	...
	this->dxWrapper->d9Wrapper->setDevice(device);   // set the device to d3d wrapper
	this->dxWrapper->d9Wrapper->setHWND(hwnd);
	...
	if(dxWrapper->d9Wrapper->init(image, hwnd) < 0){
		infoRecorder->logError("[VideoGenerator]: init d3d wrapper failed.\n");
		return -1;
	}
#ifdef SOURCES
	this->setupImagePipeline(image);
...
void VideoGenerator::run(){
	...
	if(!this->isD3DGame()){
		...
		if(useSourceType == SOURCE_TYPE::IMAGE){
			Wrapper * base = dxWrapper->d10Wrapper;
...
#ifdef DEBUG
		if(presentEvent == NULL){
			infoRecorder->logError("[VideoGenerator]: NULL present event for 3D game.\n");
		}
#endif
		if(presentEvent){
		...
		if(dxVersion == DX9){
			...
			dxWrapper->d11Wrapper(captureTv, initialTv, freq, frameInterval);
		}
...
int VideoGenerator::startModules(){
	if(useEncoderType == X264_ENCODER){
		if(x264Encoder){
			...
			if(filter == NULL){
				infoRecorder->logError("[VideoGenerator]: startModules get NULL filter for X264Encoder.\n");
				return -1;
			}
			//
			if(!filter->isRunning()){
			...
			//error
			infoRecorder->logError("[VideoGenerator]: startModules get NULL x264Encoder.\n");
			return -1;
		}
	}
	else if(useEncoderType == CUDA_ENCODER){
		if(cudaEncoder == NULL){
			infoRecorder->logError("[VidoeGenerator]: startModules get NULL cudaEncoder.\n");
			return -1;
		}
		if(!cudaEncoder->isStart()){
			// start the encoder
	...
	if(supportD3D){
		// use DXWrapper
		// check wrapper
		if(dxWrapper->d9Wrapper == NULL){
			infoRecorder->logError("[VideoGenerator]: startModules get NULL DXWrapper.\n");
			return -1;
		}
		...
		if(windowWrapper == NULL){
			infoRecorder->logError("[VideoGenerator]: startModules get NULL windowWrapper.\n");
		}
	}
}
...
void VideoGenerator::onThreadStart(LPVOID param){
	...
	if(!imageSource[0] || !imageDst[0]){
		infoRecorder->logError("[VideoGenerator]: image source or destination pipeline is NULL.\n");
	}
	...
	if(this->isD3DGame()){
		if(!surfaceSource[0] || !surfaceDst[0]){
			infoRecorder->logError("[VideoGenerator]: surface source or destination pipeline is NULL.\n");
		}
	}
	else{
		// 2D games
		if(getCurSourceType() == SURFACE){
			infoRecorder->logError("[VideoGenerator]: 2D game does not support SURFACE.\n");
			return;
		}
	}
...
int VideoGenerator::changeEncoderDevice(ENCODER_TYPE type){
	...
	if(type == X264_ENCODER){
		if(this->filter){
			...
			// error
			infoRecorder->logError("[VideoGenerator]: use x264Encoder with NULL filter.\n");
			return -1;
		}
		if(!x264Encoder->isStart()){
			x264Encoder->start();
		...
		// invalid
		infoRecorder->logError("[VideoGenerator]: invalid encoder type to change.\n");
		return -1;
	}
	return 0;
}