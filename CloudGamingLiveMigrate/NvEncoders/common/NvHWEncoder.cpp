...
namespace cg{
	namespace nvenc{
...
#else
			GUID inputCodecGUID = NV_ENC_CODEC_H264_GUID;
#endif 
		...
		NVENCSTATUS CNvHWEncoder::ProcessOutput(const EncodeBuffer *pEncodeBuffer, cg::VideoWriter * writer)
		{
			NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
#if 1
				if(buf == NULL){
					buf = (char *) malloc(sizeof(char) * 1024 * 1024 * 2);
				}
				if(lockBitstreamData.bitstreamSizeInBytes > (1024 * 1024 * 2 - 4)){
					// overflow
					
				}

				// copy frame data to buf
				memcpy(buf + 4, lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes);
				AVPacket pkt;
				av_init_packet(&pkt);
				pkt.data = (uint8_t*)(buf + 4);
				pkt.size = lockBitstreamData.bitstreamSizeInBytes;
#else
				AVPacket pkt;
				pkt.data = (uint8_t*)lockBitstreamData.bitstreamBufferPtr;
				pkt.size = lockBitstreamData.bitstreamSizeInBytes;
#endif
				writer->sendPacket(0, &pkt, writer->getUpdatedPts());
		...
		NVENCSTATUS CNvHWEncoder::NvEncEncodeFrame(EncodeBuffer *pEncodeBuffer, NvEncPictureCommand *encPicCommand, uint32_t width, uint32_t height, NV_ENC_PIC_STRUCT ePicStruct, int8_t *qpDeltaMapArray, uint32_t qpDeltaMapArraySize)
		{
			NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
