...
//#include "..\..\CloudGamingLiveMigrate\LibVideo\Generator.h"
#include "..\..\CloudGamingLiveMigrate\LibCore\InfoRecorder.h"
...
void onDeviceCreation(IDirect3DDevice9 * device, int height, int width, HWND hwnd){
	...
	if(generator == NULL){
		// error
		infoRecorder->logTrace("create generator failed.\n");
		return;
	}

	cudaError ret;
	if((ret = cudaD3D9SetDirect3DDevice(device))!= cudaSuccess){
		infoRecorder->logTrace("[cudaD3D9SetDirect3DDevice failed.\n");
	}
...
void onPresent(){
	...
	// set the event
	infoRecorder->logTrace("trigger the present event.\n");
