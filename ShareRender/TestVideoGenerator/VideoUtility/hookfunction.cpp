...
hook_d3d(UINT SDKVersion)
{
	...
	if(!deviceFound){
		infoRecorder->logError("[HOOK]: No CUDA-compatible Direct3D9 device available\n");
		pDirect3D9->Release();
		exit(EXIT_SUCCESS);
	}
...
#if 0
	if(d3dMutex == NULL){
		infoRecorder->logError("[HOOK]: to create d3d mutex.\n");
		d3dMutex = CreateMutex(NULL, FALSE, NULL);


	}else{
		infoRecorder->logError("[HOOK]: d3dMutex not NULL.\n");
	}

	
	if(nvencMutex == NULL){
		infoRecorder->logError("[HOOK]: to create nvenc mutex.\n");
		nvencMutex = CreateMutex(NULL, FALSE, NULL);


	}else{
		infoRecorder->logError("[HOOK]: nvencMutex not NULL.\n");
	}
#endif
	...
	)
{
	...
	infoRecorder->logError("[HOOK]: D3D9CreateDevice called. D3D9Device:0x%p\n", *ppReturnedDeviceInterface); 
	...
	if(generator){
		...
		if((ret = cudaD3D9SetDirect3DDevice(*ppReturnedDeviceInterface)) != cudaSuccess){
			infoRecorder->logError("[Hook}; cudaD3D9SetDirectDevice failed with :%d.\n", ret);
		}
		else{
			infoRecorder->logError("[Hook]: cudaD3D9SetDirect3DDevice success.\n");
		}
		...
		infoRecorder->logError("[HOOK]: get NULL generator.\n");
	}
	...
	)
{
	...
	//infoRecorder->onFrameEnd();
	onPresent();
	infoRecorder->logError("[HOOK]: d3d9 present, present event:%p.\n", generator->getPresentEvent());
	SetEvent(generator->getPresentEvent());
	////////////////////////////////////////////////////
	//QueryPerformanceCounter(&frame_end);

#if 0
	infoRecorder->logError("[HOOK]: lock d3d. main thread blocked.\n");
	WaitForSingleObject(d3dMutex, INFINITE);
	infoRecorder->logError("[HOOK]: unlock nvenc.\n");
