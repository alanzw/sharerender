...
void X264Encoder::InitEncoder(RTSPConf * rtspConf, pipeline *p){
	...
	//
	if (pipe == NULL) {
		Log::log("video encoder: NULL pipeline specified.\n");
		goto video_quit;
	}
	...
	//
	Log::log("video encoder: image source from '%s' (%dx%d) via channel %d, resolution=%dx%d.\n",
	...
	if (codecContext == NULL) {
		Log::slog("video encoder: cannot initialized the encoder.\n");
		goto video_quit;
	}
	//
	...
	if (ve_malloc(nalbuf_size, (void**)&nalbuf, &nalign) < 0) {
		Log::log("video encoder: buffer allocation failed, terminated.\n");
		goto video_quit;
	}
	nalbuf_a = nalbuf + nalign;
	//
	if ((pic_in = avcodec_alloc_frame()) == NULL) {
		Log::log("video encoder: picture allocation failed, terminated.\n");
		goto video_quit;
	}
	pic_in_size = avpicture_get_size(PIX_FMT_YUV420P, outputW, outputH);
	if ((pic_in_buf = (unsigned char*)av_malloc(pic_in_size)) == NULL) {
		Log::log("video encoder: picture buffer allocation failed, terminated.\n");
		goto video_quit;
	}
	...
	// start encoding
	Log::log("video encoding started: tid=%ld %dx%d@%dfps, nalbuf_size=%d, pic_in_size=%d.\n",
		GetCurrentThreadId(),
...
AVCodecContext * X264Encoder::InitEncoder(AVCodecContext * ctx, AVCodec *codec, int width, int height, int fps, vector<string> *vso){
	...
	if (vso != NULL) {
		...
		for (i = 0; i < n; i += 2) {
			...
			Log::log("vencoder-init: option %s = %s\n",
				(*vso)[i].c_str(),
				(*vso)[i + 1].c_str());
		}
	}
	else {
		Log::slog("vencoder-init: using default video encoder parameter.\n");
	}
...
void X264Encoder::run(){
	...
	Log::log("[X264Encoder]: encoder pipe loader data.\n");
	...
	if (data == NULL){
		...
		Log::log("[X264Encoder]: encoder pipe wait data...\n");
		if ((err = sourcePipe->timedwait(cond, condMutex, &to)) != 0){
			Log::slog("video encoder: image source timed out. \n");
			return;
		}
		data = sourcePipe->load_data();
		if (data == NULL){
			Log::slog("video encoder: unexpected NULL frame recvived (from '%s', data = %d, buf=%d).\n",
	...
	if (pic_in->linesize[0] == frame->lineSize[0] &&
		...
		Log::slog("video encoder: YUV mode failed - mismatched linesize(s) (src: %d,%d,%d; dst: %d,%d,%d\n",
	...
	Log::slog("[X264Encoder]: the packet pts is :%d.\n", pts);

	pic_in->pts = pts;
	av_init_packet(&pkt);
	pkt.data = nalbuf_a;
	pkt.size = nalbuf_size;
	Log::log("[X264Encoder]: to encode frame data.\n");
	if (avcodec_encode_video2(codecContext, &pkt, pic_in, &got_packet) < 0){
		Log::slog("video encoder: encoder failed, terminated.\n");
		return;
	}
	if (got_packet){
		...
		if (video_written == 0){
			video_written = 1;
			Log::log("first video frame written (pts=%lld)\n", pts);
		}
	}
	else{
		Log::log("[X264Encoder]: get no data packet.\n");
	}
}
...
void X264Encoder::onThreadStart(LPVOID param){
	...
	// init variables
	if (sourcePipe == NULL){
		Log::slog("source pipe for the encoder is NULL\n");
		return;
	}
	...
	Log::log("[X264Encoder]: rtp_id is %d.\n", rtp_id);
	...
	if (ve_malloc(nalbuf_size, (void **)&nalbuf, &nalign) < 0){
		Log::slog("video encoder: buffer allocation failed, terminated.\n");
	...
	if ((pic_in_buf = (unsigned char *)av_malloc(pic_in_size)) == NULL){
		Log::slog("video encoder: picture buffer allocation failed, terminated!\n");
		return;
	}
	avpicture_fill((AVPicture *)pic_in, pic_in_buf, PIX_FMT_YUV420P, iwidth, iheight);
	// start encoding
	Log::slog("video encoding started: tid=%ld %dx%d@%dfps, nabibuf_size = %d, pic_in_size = %d.\n",
		ccg_gettid(),
...
void X264Encoder::onQuit(){
	...
	Log::slog("X264 encoder: thread terminatedf (tid=%ld).\n", ccg_gettid());
	return;
}
...
int X264Encoder::sendPacket(int channelId, RTSPContext * rtsp, AVPacket *pkt, int64_t encoderPts){
	...
	if (rtsp->state != SERVER_STATE_PLAYING){
		Log::log("[X264Encoder]: server state is not SERVER_STATE_PLAY, is %d!\n", rtsp->state);
	}

	if (rtsp->fmtCtx[channelId] == NULL){
		// not initialized -  disable?
		Log::log("[X264Encoder]: channel %d fmtCtx is NULL.\n", channelId);
		return 0;
	}
	...
	if (ffio_open_dyn_packet_buf(&rtsp->fmtCtx[channelId]->pb, rtsp->mtu) < 0){
		Log::slog("x264Encoder %d: buffer allocation failed\n", channelId);
		return -1;
	}
	if (av_write_frame(rtsp->fmtCtx[channelId], pkt) != 0){
		Log::slog("x264Encoder %d: RTSP write failed!\n", channelId);
		return -1;
	}
	...
	if (rtsp->lowerTransport[channelId] == RTSP_LOWER_TRANSPORT_TCP){
		...
		if (rtsp->rtpWriteBinData(channelId, iobuf, ioLen, this->frameIndex) < 0){
			av_free(iobuf);
			Log::slog("x264Encoder %d: RTP write failed.\n", channelId);
			return -1;
		}
	}
	av_free(iobuf);
...
DWORD WINAPI X264ThreadFunc(PVOID param){
	...
	int video_written = 0;
	if (pipe == NULL){
		Log::slog("video encoder: NULL pipeline specified.\n");
		goto quit1;
	}
	...
	if (encoder == NULL){
		Log::slog("video encoder: cannot initialization the encoder.\n");
		goto quit1;
	}
	// 
	nalbuf_size = 100000 + 12 * iwidth * iheight;
	...
	if (encoderManager == NULL){
		Log::slog("video encoder: get the encoder manager failed\n");
		goto quit1;
	}
