...
int Filter::init(void * arg)
{
	...
	if (srcpipe == NULL){
		Log::slog("RGB2YUV filter: init - NULL pipeline specified (%s). \n", filterpipe[0]);
		goto init_failed;
	}
	...
	if (conf->confReadInts("output-resolution", resolution, 2) == 2){
		...
		Log::log("[Filter]: output-resolution specified: %dx%d.\n", outputW, outputH);
	}
	...
	if (swsctx == NULL){
		Log::slog("RGB2YUV filter: connot initialize swsscale.\n");
		goto init_failed;
	}

	if ((pipe = new pipeline()) == NULL){
		Log::slog("RGB2YUV filter: init pipeline failed.\n");
		goto init_failed;
	}
	...
	if (srcpipe->get_privdata_size() > 0){
		if (pipe->alloc_privdata(srcpipe->get_privdata_size()) == NULL){
			Log::slog("RGB2YUV filter: cannot allocate privdata.\n");
			goto init_failed;
		}
	...
	if ((data = pipe->datapool_init(POOLSIZE, sizeof(struct ImageFrame))) == NULL){
		Log::slog("RGB2YUV filter: cannot allocate data pool.\n");
		goto init_failed;
	}
	// per frame init
	for (; data != NULL; data = data->next){
		...
		if (frame->init(iwidth, iheight, istride) == NULL){
			Log::slog("RGB2YUV filter: init frame failed!\n");
			goto init_failed;
		}
	}
...
int Filter::init(int iheight, int iwidth, int outH, int outW){
	Log::log("[Filter]: init the filter.\n");
	struct SwsContext * swsctx = NULL;
	RTSPConf * conf = RTSPConf::GetRTSPConf();
	...
	// check
	if(swsctx == NULL){
		Log::slog("[Filter]: cannot initialize swsscale.\n");
		return -1;
	}
	return 0;
}
...
DWORD WINAPI FilterThreadProc(LPVOID param)
{
	...
	// param is the filter
	Filter * filter = NULL;
	Log::log("[Filter]: thread proc param[0]:%s, param[1]:%s\n", filterpipe[0], filterpipe[1]);
	...
	if (srcpipe == NULL){
		Log::log("RGB2YUV fitler: NULL pipeline specified.\n");
		goto filter_quit;
	}
	// init variables
#if 0
...
#endif
		Log::slog("RGB2YUV filter: cannot find filter pipeline \n");
		goto filter_quit;
	}
	//if ((dstpipe = pipeline::lookup(filterpipe[1])) == NULL){
	//	Log::slog("RGB2YUV filter: cannot find pipeline '%s'\n", filterpipe[1]);
	//	goto filter_quit;
	//}
	if ((filter = Filter::lookup(filterpipe[1])) == NULL){
		Log::slog("[Filter]: cannot find the filter.\n");
		goto filter_quit;
	}

	Log::log("RGB2YUV filter: pipe from '%s' to '%s' (output-resolution=%dx%d)\n",
	...
	while (true){
		...
		if (srcdata == NULL){
			...
			srcdata = srcpipe->load_data();
			if (srcdata == NULL){
				Log::slog("RGB2YUB fitler: unexpected NULL frame received (from '%s', data = %d, buf = %d).\n",
		...
		if (srcframe->pixelFormat == PIX_FMT_RGBA ||
			srcframe->pixelFormat == PIX_FMT_BGRA){
				...
				if (swsCtx == NULL){
					Log::log("RGB2YUV filter: fatal - cannot create frame conventer (%d, %d)->(%x, %d).\n",
		...
		{
			Log::log("[filter]: src frame format not supported.\n");
		}
		srcpipe->release_data(srcdata);
		dstpipe->store_data(dstdata);
		Log::log("[Filter]: notify encoder.\n");
		dstpipe->notify_all();
	}
...
filter_quit:
	...
	Log::log("RGB2YUV filter: thread terminated!\n");
	return NULL;
}
...
int Filter::do_register(const char * provider, Filter * filter){
	...
	if (filterMap.find(provider) != filterMap.end()){
		...
		Log::slog("[filter]: deplicated filter '%s'\n", provider);
		return -1;
	}
	filterMap[provider] = filter;
	ReleaseMutex(filterMutex);
	filter->myname = provider;
	Log::log("[filter]: new filter '%s' registered.\n", provider);
	return 0;
}
void Filter::do_unregister(const char * provider){
	...
	ReleaseMutex(filterMutex);
	Log::log("[filter]: filter '%s' unregistered.\n", provider);
	return;
}
...
void Filter::run(){
	...
	if(srcdata == NULL){
		...
		if(srcdata == NULL){
			Log::slog("Filter]: get NULL src data from source pipeline .\n");
			return;
		}
	}
	...
	else if(getOutputForamt() == PIX_FMT_NV12){
		dstframe->pixelFormat = PIX_FMT_NV12;
	}
	...
	if(iframe->pixelFormat == PIX_FMT_RGBA || iframe->pixelFormat == PIX_FMT_BGRA){
		if(swsCtx == NULL){
			...
			if(swsCtx == NULL){
				Log::slog("[Filter]: fatal - cannot create frame conventer.\n");
		...
		Log::slog("[Filter]: src frame format is not supported.\n");
	}
	srcPipe->release_data(srcdata);
	dstPipe->store_data(dstdata);
	Log::log("[Filter]: notify encoder.\n");
...
void Filter::onQuit(){
	...
	Log::log("[Filter]: thread terminated.\n");
}
...
void Filter::onThreadStart(){
	if(!this->isInited()){
		Log::slog("[Filter]: enter thread proc without initialization.\n");
		return;
	}

	if(srcPipe == NULL){
		Log::slog("[Filter]: without source pipeline .\n");
		return;
	}
	if(dstPipe == NULL){
		Log::slog("[Filter]: without destination pipeline.\n");
		return;
	}
