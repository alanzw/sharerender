	...
	if(ret != cudaSuccess){
		...
		for(int i= 0; i < this->rows; i++){
			for(int j = 0; j < this->cols; j++){
				...
				//infoRecorder->logError("(%d,%d,%d,%d) ", dataToSave[i * this->step + j * this->elemSize() + 0], dataToSave[i * this->step + j * this->elemSize() + 1], dataToSave[i * this->step + j * this->elemSize() + 2], dataToSave[i * this->step + j * this->elemSize() + 3]); 
			}
