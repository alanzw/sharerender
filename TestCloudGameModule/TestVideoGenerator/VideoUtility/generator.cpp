...
int VideoGen::initVideoGen(DX_VERSION  ver, void * device){
	infoRecorder->logError("[VideoGen]: initVideoGen called.\n");
	dxVer = ver;
	...
	if(dxVer == DX9){
		...
		infoRecorder->logError("[VideoGen]: directx version is D3D9.\n");
		...
		infoRecorder->logError("[VideoGen]: directx version is D3D10.\n");
		...
		infoRecorder->logError("[VideoGen]: directx version is D3D10_1.\n");
		swapChain = (IDXGISwapChain *)device;
		dxWrapper->d10Wrapper1 = new D3D10Wrapper1(swapChain);
		dxWrapper->d10Wrapper1->registerCudaSource(cudaPipe);
	}
	else if(dxVer == DX11){
		infoRecorder->logError("[VideoGen]: directx version is D3D11.\n");
		swapChain = (IDXGISwapChain *)device;
...
int VideoGen::setupSurfaceSource(){
	...
	infoRecorder->logError("[VideoGen]: setupSurfaceSource called.\n");
...
BOOL VideoGen::stop(){
	//DebugBreak();
	infoRecorder->logError("[VideoGen]: stop called.\n");
	return CThread::stop();
}
// the loop logic
void VideoGen::run(){
	...
	//infoRecorder->logError("[VideoGen]: run called. present event:%p\n", presentEvent);
...
void VideoGen::onThreadMsg(UINT msg, WPARAM wParam, LPARAM lParam){
	infoRecorder->logError("[VideoGen]: onThreadMsg called.\n");
}
// release
void VideoGen::onQuit(){
	infoRecorder->logError("[VideoGen]: onQuit called.\n");
}
// init before enter loop
void VideoGen::onThreadStart(){
	// start the modules
	infoRecorder->logError("[VideoGen]: onThreadStart called\n");
	if(dxWrapper->d10Wrapper == NULL){
		// error
		infoRecorder->logError("[VideoGen]: get NULL wrapper.\n");
	}
	if(cudaPipe == NULL){
		//error
		infoRecorder->logError("[VideoGen]: get NULL cuda pipe.\n");
	}
	if(useNvenc == false){
...
int VideoGen::initCudaEncoder(void * device, void * context){
	infoRecorder->logError("[VideoGen]: D11 init the cuda encoder.\n");
...
int VideoGen::initNVENCEncoder(void * device){
	dxDevice->d10Device = (ID3D10Device *)device;
	infoRecorder->logError("[VideoGen]: init the NVENC encoder.\n");
	// error
	char name[100] = {0};
	...
	infoRecorder->logError("[VideoGen]: before init cuda filter, width:%d, height:%d.\n", this->width, this->height);
...
int VideoGen::initCudaEncoder(void * device){
	...
	dxDevice->d10Device = (ID3D10Device *)device;
	infoRecorder->logError("[VideoGen]: init the cuda encoder.\n");
	// error
	char name[100] = {0};
