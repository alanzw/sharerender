...
bool ParseEncoderParams(NVEncoderParams * pParams){
	...
	infoRecorder->logError("[CudaEncoder]: ParseEncoderParams called.\n");
	...
	Log::slog("[CudaEncoder]: current directory is: %s.\n", path);
...
bool CudaEncoder::baiscSetup(){
	cuFilter = NULL;
	cond = NULL;
	infoRecorder->logError("[CudaEncoder]: basicSetup called.\n");
	HRESULT hr = S_OK;
	...
	infoRecorder->logError("[CudaEncoder]: configuration file: '%s'.\n", this->m_pEncoderParams->configFile);
	...
	infoRecorder->logError("[CudaEncoder]: encoded output file: '%s'.\n", this->m_pEncoderParams->outputFile);
	infoRecorder->logError("[CudaEncoder]: measurement: %s.\n", "(FPS) frame per second");
	...
	{
		infoRecorder->logError("[CudaEncoder]: fopen() error - the oiutput file '%s' could not be created for writing.\n", this->m_pEncoderParams->outputFile);
	...
	{
		infoRecorder->logError("[CudaEncoder]: GetEncoderParamsConfig() error.\n");
...
CudaEncoder::CudaEncoder(int ih, int iw, int oh, int ow, char * outputFileName):frameSize_(ow, oh){
	//NVEncoderParams  params = {0};
	if(outputFileName)
		infoRecorder->logError("[CudaEncoder]: constructor called. output file name:%s.\n", outputFileName);
	else{
		infoRecorder->logError("[CudaEncoder]: constructor called. output file not specificed.\n");
	}
	...
	m_pEncoderParams->iUseDeviceMem = 1;
	infoRecorder->logError("[CudaEncoder]: constructor finished.\n");
...
CudaEncoder::~CudaEncoder()
{
	infoRecorder->logError("[CudaEncoder]: destructor called.\n");
#if 0
	if (fpIn){
		fclose(fpIn);
	}
...
{
	infoRecorder->logError("[CudaEncoder]: initEncoder called.\n");
	// Create the Encoder API Interface
	HRESULT hr = NVCreateEncoder((NVEncoder *)&m_pEncoder);
	infoRecorder->logError("[CudaEncoder]: NVCreateEncoder <%s>, hr = %08x\n", (FAILED(hr) ? "FAILED!" : "SUCCESS!"), hr);
...
bool CudaEncoder::initGpuMemory(){
	int err;
	infoRecorder->logError("[CudaEncoder]: initGpuMemory, size.height:%d, size.width:%d.\n", frameSize_.height, frameSize_.width);
	// init context
...
bool CudaEncoder::SetEncodeParameters()
{
	...
	infoRecorder->logError("[CudaEncoder]: setEncoderParameters called.\n");
	...
	infoRecorder->logError("[CudaEncoder]: setEncoderParameters <%s>, bRetVal = %08x\n", ((bRetVal != TRUE) ? "FAILED!" : "SUCCESS!"), bRetVal);
...
{
	infoRecorder->logError("[CudaEncoder]: setCBFunctions called. pCB:0x%p, userdata:0x%p.\n", pCB, pUserData);
	if (pCB)
	{
...
#else
		infoRecorder->logError("[CudaEncoder]: NULL NVVE_CallbackParams.\n");
		return false;
#endif
	}
...
bool CudaEncoder::CreateHWEncoder(NVEncoderParams *pParams)
{
	...
	infoRecorder->logError("[CudaEncoder]: CreateHWEncoder called. <%s>, hr = %08x.\n", (FAILED(hr) ? "FALED! Unable to create NVIDIA HW Video Encoder": "OK!"), hr);
	...
	if (FAILED(hr))
	{
		...
		infoRecorder->logError("[CudaEncoder]: NVGetSPSPPS error getting SPSPPS buffer.\n");
	}
	else
	{
		printf("VideoEncoder() - NVGetSPSPPS <%s>, hr = %08x\n", (FAILED(hr) ? "FAILED!" : "OK!"), hr);
		infoRecorder->logError("[CudaEncoder]: NVGetSPSPPS <%s>, hr = %08x.\n", (FAILED(hr) ? "FAILED!" : "OK!"), hr);
	}
...
size_t CudaEncoder::ReadNextFrame(CUdeviceptr dstPtr){
	...
	infoRecorder->logError("[CudaEncoder]: ReadNextFrame called.\n");
	// the source is only the imageSourcePipe
	if(cudaPipe == NULL){
		infoRecorder->logError("[CudaEncodedr]: get NULL cuda soruce pipeline .\n");
		return 0;
	}
	data = cudaPipe->load_data();
	if(data == NULL){
		infoRecorder->logError("[CudaEncoder]: wait the surface event.\n");
		int err;
		//if ((err = cudaPipe->timedwait(this->sourceNotifier, condMutex, &to)) != 0){
		if((err = cudaPipe->wait(this->sourceNotifier, condMutex))!=0){
			Log::slog("[CudaEncoder]: image source timed out.\n");
			infoRecorder->logError("[CudaEncoder]: surface source timed out.\n");
			return -1;
		}
		data = cudaPipe->load_data();
		if (data == NULL){
			infoRecorder->logError("[CudaEncoder]: unexpected NULL frame received ( from '%s', data = %d, buf = %d.\n", cudaPipe->name(), cudaPipe->data_count(), cudaPipe->buf_count());
			return -1;
		}
		else{
			infoRecorder->logError("[CudaEncoder]: wait and get a surface.\n");
		}
	}
	...
	infoRecorder->logError("[CudaEncoder]: read a surface.\n");
	...
	if(vf->type == IMAGE){
		infoRecorder->logError("[CudaEncoder]: get a IMAGE source.\n");
		...
		infoRecorder->logError("[CudaEncoder]: get a SURFACE source.\n");
		...
		if(!cuFilter->updateSurface(sframe->dxSurface)){
			infoRecorder->logError("[CudaEncoder]: update surface failed.\n");
			return -1;
		}

		// run the cuda kernel to convert format
		if(dstPtr != NULL){
			infoRecorder->logError("[CudaEncoder]: call filter to covert surface format.\n");
		...
		infoRecorder->logError("[CudaEncoder]: get an NUKNOWN source, surface: %p, type:%d, type addr:%p.\n", vf, vf->type, &(vf->type));
		return -1;
	}
}
...
bool CudaEncoder::EncodeFrame(NVVE_EncodeFrameParams &sFrameParams, CUdeviceptr dptr_VideoFrame, CUvideoctxlock ctxLock)
{
	...
	infoRecorder->logError("[CudaEncoder]: EncodeFrame called.\n");
	...
	if (m_pEncoderParams->iUseDeviceMem)
	{
		if (inputType == CPUIMAGE){
			...
			infoRecorder->logError("[CudaEncoder]: encoder input type is GPUIMAGE. device ptr:0x%p\n", dptr_VideoFrame);
		...
		infoRecorder->logError("[CudaEncoder]: brefore call NVEncodeFrame.\n");
	...
	if(hr == S_OK){
		infoRecorder->logError("[CudaEncoder]: encode frame success.\n");
	}else if(hr == E_FAIL){
		infoRecorder->logError("[CudaEncoder]: encode frame failed.\n");

	}else if(hr == E_POINTER){
		infoRecorder->logError("[CudaEncoder]: encoder handle is invalid.\n");
	}

	if (FAILED(hr))
	{
		printf("VideoEncoder::EncodeFrame() error when encoding frame (%d)\n", m_nFrameCount);
		infoRecorder->logError("[CudaEncoder]: encode frame error when encoding frame (%d).\n", m_nFrameCount);
...
{
	infoRecorder->logError("[CudaEncoder]: GetEncodeParamsConfig called.\n");
	//assert(pParams != NULL);

	//DebugBreak();
	int iAspectRatio = 0;
	if (pParams == NULL){
		infoRecorder->logError("[CudaEncoder]: get NULL pParams.\n");
		return false;
	}
...
HRESULT CudaEncoder::SetParamValue(DWORD dwParamType, void *pData)
{
	...
	//infoRecorder->logError("[CudaEncoder]: SetParamValue called.\n");
	if (hr != S_OK)
	{
		infoRecorder->logError("  NVSetParamValue: %26s,  hr = %08x ", sNVVE_EncodeParams[dwParamType].name, hr);

		for (int i = 0; i < sNVVE_EncodeParams[dwParamType].params; i++)
		{
			infoRecorder->logError(" %8d", *((DWORD *)pData + i));
			infoRecorder->logError(", ");
		}

		infoRecorder->logError(" FAILED!");
	}
	else
	{
		infoRecorder->logError("  NVSetParamValue : %26s = ", sNVVE_EncodeParams[dwParamType].name);
	...
	case NVVE_FIELD_ENC_MODE:
		infoRecorder->logError(" [%s]", sPictureType[*((DWORD *)pData)]);
		break;

	case NVVE_RC_TYPE:
		infoRecorder->logError(" [%s]", sNVVE_RateCtrlType[*((DWORD *)pData)]);
		break;

	case NVVE_PRESETS:
		infoRecorder->logError(" [%s Profile]", sVideoEncodePresets[*((DWORD *)pData)]);
		break;
	...
	case NVVE_GPU_OFFLOAD_LEVEL:
		...
		case NVVE_GPU_OFFLOAD_DEFAULT:
			infoRecorder->logError(" [%s]", sGPUOffloadLevel[0]);
			break;

		case NVVE_GPU_OFFLOAD_ESTIMATORS:
			infoRecorder->logError(" [%s]", sGPUOffloadLevel[1]);
			break;

		case 16:
			infoRecorder->logError(" [%s]", sGPUOffloadLevel[2]);
			break;
		}
...
{
	assert(pParams != NULL);
	infoRecorder->logError("[CudaEncoder]: SetCodecType called.\n");
	HRESULT hr = S_OK;
	...
	infoRecorder->logError("  NVSetCodec ");

	if (pParams->iCodecType == 4){
		infoRecorder->logError("<H.264 Video>\n");
	}
	else if (pParams->iCodecType == 5){
		// Support for this codec is being deprecated
		infoRecorder->logError("<VC-1 Video> (unsupported)\n");
		return false;
	}
	else{
		infoRecorder->logError("Unknown Video Format \"%s\"\n", pParams->iCodecType);
		return false;
	}
...
{
	assert(pParams != NULL);
	//infoRecorder->logError("[CudaEncoder]: setParameters called.\n");
...
{
	assert(pParams != NULL);
	infoRecorder->logError("[CudaEncoder]: setActiveGPU called.\n");
	infoRecorder->logError("  >> Setting Active GPU %d for Video Encoding <<\n", gpuID);
	HRESULT hr = SetParamValue(NVVE_FORCE_GPU_SELECTION, &gpuID);

	if (hr != S_OK)
	{
		infoRecorder->logError("  >> NVVE_FORCE_GPU_SELECTION Error <<\n\n");
	}
...
CudaEncoder * CudaEncoderFactory::CreateCudaEncoder(const char * configFile, INPUT_TYPE inputType){
	if (!factory){
		Log::slog("NULL Cuda Encoder Factory!\n");
		return NULL;
	}
	else{
...
{
	infoRecorder->logError("[CudaEncoder]: HnadleOnBeginFrame called, frame number:%d.\n", pbfi->nFrameNumber);
	/// set the bitrate of the encoder
	return;
}
...
static unsigned char * _stdcall HandleAcquireBitStream(int * pBufferSize, void * pUserData){
	...
	infoRecorder->logError("[CudaEncoder]: HandelAcquireBitStream called. encoder:0x%p.\n", pUserData);
	CudaEncoder * encoder;
	if (pUserData){
		...
		// error
		Log::slog("Cuda encoder: null encoder specified\n");
		return NULL;
	}
...
static void _stdcall HandleReleaseBitStream(int nBytesInBuffer, unsigned char * cb, void * pUserData){
	infoRecorder->logError("[CudaEncoder]: HandleReleaseBitStream called.\n");
	...
	if (pUserData){
		...
		// error
		infoRecorder->logError("[ERROR]: null encoder specified\n");
		return;
	}
#if 0
	if (encoder){
	...
	// write to file
	if(encoder->fileOut()){
		infoRecorder->logError("[CudaEncoder]: write to output:%d.\n", nBytesInBuffer);
		fwrite(cb, 1, nBytesInBuffer, encoder->fileOut());
	}
	else{
		infoRecorder->logError("[CudaEncoder]: file out pointer is NULL.\n");
	}
#endif
	return;
}
...
static void _stdcall HandleOnEndFrame(const NVVE_EndFrameInfo * pefi, void *pUserData){
	infoRecorder->logError("[CudaEncoder]: HandleOnEndFrame called.\n");
	CudaEncoder * encoder = NULL;
	if (pUserData){
		encoder = (CudaEncoder*)pUserData;
		...
		//error
		infoRecorder->logError("[CudaEncoder]: null encoder specified\n");
		Log::slog("Cuda encoder: null encoder specified\n");
		return;
	}
	encoder->frameSummation(-(pefi->nFrameNumber));
	if (encoder->IsLastFrameSent()){
		infoRecorder->logError("[CudaEncoder]: get last frame.\n");
		// check to see if the last frame has been sent
		if (encoder->getFrameSum() == 0){
			//
		}
...
void CudaEncoder::run(){
	infoRecorder->logError("[CudaEncoder]: run called.\n");
	...
	if ((inputType == CPUIMAGE || !sEncoderParams.iDeviceMemInput)){
		...
		infoRecorder->logError("[CudaEncoder]: input type is GPU image, ater get next frame.\n");
	}
	else{
		// error
		exit(EXIT_SUCCESS);
	...
	if (EncodeFrame(efparams, dptrVideoFrame, cuCtxLock) == false){
		// error
		Log::slog("[CudaEncoder]: failed to encoder frame.\n");
		infoRecorder->logError("[CudaEncoder]: failed to encode frame, do not know.\n");
	}
	else{
		infoRecorder->logError("[CudaEncoder]: success encode frame.\n");
		Log::slog("[CudaEncoder]: success encode frame.\n");
	}

	computeFPS();
}
void CudaEncoder::onThreadMsg(UINT msg, WPARAM wParam, LPARAM lParam){
	infoRecorder->logError("[CudaEncoder]: onThreadMsg called.\n");
}
void CudaEncoder::onThreadStart(){
	infoRecorder->logError("[CudaEncoder]: onThreadStart called.\n");
	...
	if(inputType == INPUT_TYPE::CPUIMAGE){
		...
		if(pipe == NULL){
			Log::slog("[CudaEncoder]: image source pipeline not registered.\n");
		}
	}else if(inputType == INPUT_TYPE::GPUIMAGE){
		// find the surface source
		if(NULL == pipe){
			Log::slog("[CudaEncoder]: surface source pipe not registered.\n");
		}
	}else{
		//
		Log::slog("[CudaEncoder]: unknown source type.\n");
	...
	if(!sEncoderParams.iUseDeviceMem){
		infoRecorder->logError("[CudaEncoder]: do not use device memory.\n");
		sEncoderParams.iUseDeviceMem = 1;
	}
	if (sEncoderParams.iUseDeviceMem){

		//DebugBreak();
		infoRecorder->logError("[CudaEncoder]: use device memory.\n");
		HRESULT h = S_OK;
#if 0
		...
		infoRecorder->logError("[CudaEncoder]: after malloc device memroy, ptr:0x%p, pitch:%d, widthInBytes:%d, height:%d, ElementSizeBytes:%d.\n", dptrVideoFrame, Pitch, WidthInBytes, Height, ElementSizeBytes);
#else
...

		{
			...
			if (FAILED(hr)){
				// failed
				Log::slog("[CudaEncoder]: NVVE_DEVICE_MEMORY_INPUT failed.\n");
			}
			hr = SetParamValue(NVVE_DEVICE_CTX_LOCK, &cuCtxLock);

			if (FAILED(hr)){
				// failed
				Log::slog("[CudaEncoder]: NVVE_DEVICE_CTX_LOCK failed.\n");
			}
		}
	}
	...
	// now provide the callback functions to cuda h.264 encoder
	{
		...
		if(!SetCBFunctions(&m_NVCB, (void *)this)){
			infoRecorder->logError("[CudaEncoder]: setCBFunction failed.\n");
		}
#else
		// set the callback
		infoRecorder->logError("[CudaEncoder]: NVRegisterCB called.\n");
...
#else
	infoRecorder->logError("[CudaEncoder]: create HW encoder.\n");
...
void CudaEncoder::onQuit(){
	//Stop();
	infoRecorder->logError("[CudaEncoder]: onQuit called.\n");
...
int CudaEncoder::startEncoding(){
	infoRecorder->logError("[CudaEncoder]: startEncoding called.\n");
	return this->start();
}
...
bool CudaEncoder::InitCudaFilter(DX_VERSION ver, void * device){
	infoRecorder->logError("[CudaEncoder]: InitCudaFilter called.\n");
	if(this->cuFilter){
		infoRecorder->logError("[CudaEncoder]: cuFilter is not NULL.\n");
	}
	cuFilter = new CudaFilter(device, ver);
	if(cuFilter->initCuda(ver)==S_FALSE){
		infoRecorder->logError("[CudaEncoder]: register d3d device failed.\n");
		MessageBox(NULL, "register d3d device failed", "ERROR", MB_OK);
	}
	if(!cuFilter->initSurface(outputWidth, outputHeight)){
		infoRecorder->logError("[CudaEncoder]: init the cuda file surface failed.\n");
		return false;
	}
	//cuFilter->
...
void CudaEncoderImpl::initEncoder(double fps)
{
	...
	if (err){
		infoRecorder->logError("[CudaEncoderImpl]: StsNotImplemented, codec format is not supported.\n");
	}
